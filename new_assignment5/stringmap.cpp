...
void StringMap::rehash(int nBuckets) {
   // TODO
   KeyValuePair* odd_buckets = buckets;
   int odd_len = this->nBuckets;
   this->nBuckets = nBuckets;
   this->count = 0;
   buckets = new KeyValuePair[nBuckets];
   for (int i = 0; i < odd_len; i++) {
      if (odd_buckets[i].occupied == true) {
         string key = odd_buckets[i].key;
         string value = odd_buckets[i].value;
         put(key, value);
      }
   }
   delete[] odd_buckets;
...
int StringMap::findKey(const string & key) const {
   ...
   int time = 0;
   while (buckets[idx].occupied == true && time < nBuckets) {
      time++;
...
int StringMap::insertKey(const string & key) {
   // TODO
   int idx = hashCode(key) % nBuckets;
   if (count == nBuckets) {error("the array is full!");}
   while (buckets[idx].occupied == true && buckets[idx].key != key) {
      idx = (idx + 1) % nBuckets;
   }
