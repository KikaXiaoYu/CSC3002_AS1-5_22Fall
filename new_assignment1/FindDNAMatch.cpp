...
int findDNAMatch(string s1, string s2, int start) {
   // TODO
   unsigned int strand_len = s1.length();
   for (unsigned int i = start; i < s2.length(); i++){
      string match = matchingStrand(s1);
      string target = s2.substr(i, strand_len);
      if (match == target) return i;
   }
   return -1;
}
